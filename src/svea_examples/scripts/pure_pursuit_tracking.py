...
class pure_pursuit(rx.Node):
    ...
    def on_startup(self):
        ...
        self.goal = [self.path[self.cusp_index][0], self.path[self.cusp_index][1]]

        # self.curr = 0
        # self.goal = self._points[self.curr]
        self.goal_marker.place([*self.goal, 0.5], color='blue')
        # self.update_traj(x, y)
        self.controller.traj_x = self.path[:self.cusp_index + 1][0]
        self.controller.traj_y = self.path[:self.cusp_index + 1][1]
    ...
    def update_path(self):
        ...
        """
        self.controller.is_finished = False
        self.goal = [self.path[-1][0], self.path[-1][1]]
        self.goal_marker.place([*self.goal, 0.5], color='blue')
