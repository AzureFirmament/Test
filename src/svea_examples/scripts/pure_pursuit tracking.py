...
class pure_pursuit(rx.Node):
    ...
    def on_startup(self):
        ...
        path = pd.read_csv(r'E:\Code\PythonCode\KTH\svea\src\svea_examples\resource\tiha_path_data.csv',
                           header = 0)
        self.path = path.to_numpy()
        for i in range(len(self.path[3]) - 1):
            if self.path[3][i] > 0 and self.path[3][i + 1] < 0:
                self.cusp_index = i
                break

        self.goal = [self.path[0][self.cusp_index], self.path[1][self.cusp_index]]

        # self.curr = 0
        # self.goal = self._points[self.curr]
        self.goal_marker.place([*self.goal, 0.5], color='blue')
        # self.update_traj(x, y)
        self.controller.traj_x = self.path[0][:self.cusp_index + 1]
        self.controller.traj_y = self.path[1][:self.cusp_index + 1]
        self.path.publish_path(self.controller.traj_x, self.controller.traj_y)
    ...
    def update_path(self):
        ...
        """
        self.controller.is_finished = False
        self.goal = [self.path[0][-1], self.path[1][-1]]
        self.goal_marker.place([*self.goal, 0.5], color='blue')
        self.target_velocity = - self.target_velocity
        self.controller.traj_x = self.path[0][self.cusp_index:]
        self.controller.traj_y = self.path[1][self.cusp_index:]
        self.path.publish_path(self.controller.traj_x, self.controller.traj_y)
